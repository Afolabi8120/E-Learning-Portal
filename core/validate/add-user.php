<?php
    ...
    if(isset($_POST['btn-save-user'])){ // Save User
        ...
        if(empty($username) || empty($fullname) || empty($email) || empty($gender) || empty($phone)){
        ...
        else if($globalclass->checkIfExist('userid','tbluser',$username) === true){ # to check if staff id exist
            ...
            if($user->saveUser($username,$fullname,$gender,$mstatus,$religion,$position,$email,$phone,$dob,$newpassword,$address,'default.jpg','Active','Admin',$chat_code) === true){
                $_SESSION['SuccessMessage'] = "User account have been created successfully";
            }else{
        ...
        if($user->disableUser('tbluser','status',$user_id) === true){ // called from the User class
            $_SESSION['SuccessMessage'] = "User account have been deactivated successfully";
        }
    }else if(isset($_POST['btn-enable-user'])){ // Enable User
        $user_id = $_POST['user_id'];

        if($user->enableUser('tbluser','status',$user_id) === true){ // called from the User class
            $_SESSION['SuccessMessage'] = "User account have been activated successfully";
        }
        ...
        if($globalclass->delete('tbluser','id',$user_id)){ // called from the GlobalClass class
            $_SESSION['SuccessMessage'] = "User account have been deleted successfully";
        }
    }
